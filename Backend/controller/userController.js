...
const signIn = async (req, res,next) => {
    // console.log("signin__backend",req.body)
      ...
      try {
        ...
        res.status(200).cookie('access_token',token,option).json({name:existingUser.name,email:existingUser.email,username:existingUser.username,access_token:token});
      } catch (err) {
        console.log(err);
